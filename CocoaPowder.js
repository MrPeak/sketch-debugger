...
let animaBreakPointOffset = 0;
let animaBreakPointEnabled = true;
...
function cocoapowderBreakpoint(evalWithScope, optionalExpression, recurseCount) {
  ...
  evalWithScope = evalWithScope != undefined ? evalWithScope : function (x) {
    return eval(x);
  };
...
function animaGetCodeForTrace(stackTrace) {
  ...
  code = NSString.stringWithContentsOfFile_encoding_error_(NSString.stringWithString(filePath), 4, nil).toString();
...
var animaCreateCodeScrollingLabel = function (text, rect, breakpointRow) {
  ...
  scrollView.documentView().font = NSFont.fontWithName_size_("CourierNewPSMT", 12);
  ...
  var boldFont = NSFont.fontWithName_size_("CourierNewPS-BoldMT", 12);

  // Scroll to breakpoint and make it bold
  scrollView.documentView().setFont_range_(boldFont, NSMakeRange(rowStartIndex, rowEndIndex - rowStartIndex));
